...
create or replace trigger delete_customer_coupons
after delete on customer
for each row
begin
   delete from birthday_coupons where id = :old.id;
end;
/
--------------------------------------------------
...
--------------------------------
-- 고객의 생일이 현재 달인 경우, 생일 쿠폰을 발행하며, 그렇지 않은 경우에는 생일 쿠폰을 발행하지 않음
BEGIN
...
-- 해당 고객의 정보를 업데이트하며, 만약 고객의 생일이 현재 달인 경우 생일 쿠폰을 발행
BEGIN
...
-- 해당 고객 정보를 삭제하며, 삭제한 고객의 정보를 old_customer 테이블에 삽입
BEGIN
  p_customer_mng.delete_cust(
    p_id => 'test01'
  );
END;
/
